...
### 4.1. Диаграмма Use Case
```mermaid
graph TD
    A[Пользователь] --> B[Отправка сообщений]
    A --> C[Удаление сообщений]
    A --> D[Создание голосового канала]
    A --> E[Участие в групповом звонке]
    A --> F[Отправка медиа-файлов]
    A --> G[Управление настройками приватности]
    H[Администратор] --> D
    H --> I[Управление группой]
    H --> J[Назначение модераторов]
Формат: Markdown (готов для GitHub/Notion)
